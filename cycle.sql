...
INSERT INTO message (src, created, available, pending) SELECT id, @t, 0, 0 FROM node WHERE online = 1 AND FLOOR(RAND() * 100) = 0;
...
# create deliveries
INSERT INTO delivery (src, dst, created, received, optimal) SELECT message.src, edge.a, message.created, 0, 0 FROM message JOIN edge ON edge.b = message.src WHERE created = @t;
